...
const xpLevels = [
  ...
  1780, 1980, 2180, 2400, 2620, 2860, 3100, 3360, 3620, 3900, 4180, 4480, 4780,
  5100, 5420, 5760, 6100, 6460, 6820, 7200, 7580, 7980, 8380, 8800, 9220, 9660,
  10100, 10560, 11020, 11500, 11980, 12480, 12980, 13500, 14020, 14560, 15100,
  15660, 16220, 16800, 17380, 17980, 18580, 19200, 19820, 20460, 21100, 21760,
  22420, 23100, 23780, 24480, 25180, 25900, 26620, 27360, 28100, 28860, 29620,
  30400, 31180, 31980, 32780, 33600, 34420, 35260, 36100, 36960, 37820, 38700,
  39580, 40480, 41380, 42300, 43220, 44160, 45100, 46060, 47020, 48000, 48980,
  49980, 50980, 52000, 53020, 54060,
];
...
router.get("/rankCardGif", async (req, res) => {
  try {
    ...
    const { avatarURL, xp, level, progressBarColor, background, rank } =
      req.query;
    ...
    if (fs.existsSync(`${__dirname}/../assets/rankCardBGs/${background}`)) {
      ...
      const frames = fs
        .readdirSync(`${__dirname}/../assets/rankCardBGs/${background}`)
      ...
      for (frame of frames) {
        const ctx = await generateRankCard(
          ...
          level,
          progressBarColor,
          `${__dirname}/../assets/rankCardBGs/${background}/${frame}`,
          rank,
          candyImage
        );
...
router.post("/saveRankCard", async (req, res) => {
  try {
    ...
    const user = await xpUser.findById(
      `${session.user_id}_${config.discord.guildId}`
    );
    ...
    if (!rankCardDataJSON.progressBarColor || !rankCardDataJSON.background)
      return res.status(400).send("Missing parameters");
    console.log("valid params");

    if (!/^#[0-9A-F]{6}$/i.test(rankCardDataJSON.progressBarColor))
      return res.status(400).send("Invalid progress bar color");
    ...
    if (!/^#[0-9A-F]{6}$/i.test(rankCardDataJSON.background)) {
      if (!rankCardDataJSON.background.startsWith("#")) {
        if (
          !user.rankCard.unlockedBackgrounds.includes(
            rankCardDataJSON.background
          )
        )
          return res.status(403).send("Background not unlocked");
      }
    }
    ...
    return res
      .status(200)
      .send({ message: "Rank card data processed successfully" });
  } catch (err) {
    console.log(err);
    return res.status(500).send({ error: "Internal server error" });
  }
